...
- **Purpose:** nixify.dev is the village's bootstrap profile and developer onboarding surface. Any new machine or contributor runs `curl nixify.dev | sh` (or equivalent) to get: passwordless SSH (4 keys auto-fetched from github.com/ryzengrind.keys), Tailscale auto-connect, builder network registration, SeaweedFS + Harmonia cache prep. Zero prompts, zero manual steps. The entry point for ALL new nodes.
- **Stack:** NixOS minimal module, bash bootstrap script, Tailscale ephemeral key injection, `nixos-anywhere` compatible
- **Domain:** nixify.dev (bootstrap)
- **Canonical flake input:** `github:RyzeNGrind/nixify.dev`
- **Depends on:** `core`, `nix-cfg` (post-bootstrap target), `nixify.cloud` (cache)
- **Provides to village:** The entry point for ALL new nodes — hardware, cloud, WSL, Codespaces, any machine joining the village
...
2. Tailscale auto-connect with provided authkey (tag:compute for servers, tag:dev for workstations)
3. Builder network registration with nixify.cloud substituter and remote builder
4. SeaweedFS + Harmonia cache prep (local cache node setup)
5. Zero prompts, zero manual intervention — fully automated

## Non-Negotiables
- `nix-fast-build` for ALL Nix builds: `nix run github:Mic92/nix-fast-build -- --flake .#checks`
- Bootstrap script idempotent — safe to run multiple times without side effects
